...
(function () {
  ...
  function injectDirectButton(card) {
    ...
    btn.appendChild(
      Object.assign(document.createElement("span"), {
        textContent: "UASerials (uaserial.top)",
      })
    );
    btn.addEventListener("click", () => runForCard(card));
    const play = actions.querySelector(
      '.button[data-action="play"], .button.play, .full__button_play'
    );
  ...
  if (Lampa?.Listener?.follow) {
    Lampa.Listener.follow("full", (e) => {
      if (
        !e ||
        (e.type !== "open" && e.type !== "build" && e.type !== "render")
      )
        return;
      const card = e.object || {};
      const ok = registerMods(card);
      if (!ok) injectDirectButton(card);
    });
  }
